require(['Config'], function () {
    require(['hrmanage.addnewparticipants']);
}),
define("hrmanage.addnewparticipants", ['jquery', 'bootstrap', 'bootstrap.paginator'], function ($) {
    $(function () {
            ...
            var options = {
                ...
                onPageClicked: function (e, originalEvent, type, page) {
                    $.ajax({
                        ...
                        type: "post",
                        dataType: "json",
                        data: { "page": page, "Stage1EndDate": $("#forRazorValue").attr("stage1enddate"), "Stage3EndDate": $("#forRazorValue").attr("stage3enddate"), "EmployeeName": $("#hidEmployeeName").val(), "DomainID": $("#hidDomainID").val(), "DepartmentName": $("#hidDepartmentName").val() },
                        beforeSend: function () {
                            $('#InfoModal').modal(); ;
                        },
        ...
        $("#AddParticipants").click(function () {
            if ($("#totallist li").length == 0) {
                $("#info").text("Please select at least one record for addition.");
                $('#RedictModal').modal();
                return false;
            }
            var EIForCache = "";
            ...
            $.ajax({
                ...
                type: "post",
                dataType: "json",
                data: { "cycleDateRangeStart": $("#forRazorValue").attr("stage1enddate"), "cycleDateRangeEnd": $("#forRazorValue").attr("stage3enddate"), "EIForCache": EIForCache.substr(1) },
                beforeSend: function () {
                    $('#InfoModal').modal();
                },
                success: function (data) {
